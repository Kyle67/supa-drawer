...
export type DrawingBoardRef = {
  receivedStart: (x: number, y: number, color: string, user: string) => void;
...
const DrawingBoard = forwardRef<DrawingBoardRef, DrawingBoardProps>(
  ({ onStart, onActive }: DrawingBoardProps, ref) => {
    ...
    const onOtherDrawingStart = useCallback((x: number, y: number, color: string, user: string) => {
      setOtherPaths((old) => {
        const userPaths = old.find((obj) => obj.user === user);
    ...
    return (
      <Canvas style={style.container} onTouch={touchHandler}>
        ...
        {otherPaths.map((info, index) => (
          <Fragment key={index}>
            {info.paths.map((path, index2) => (
              <Path key={index2} path={path} color={info.color} style="stroke" strokeWidth={2} />
            ))}
          </Fragment>
        ))}
      </Canvas>
